...
from bot import APPDRIVE_EMAIL, APPDRIVE_PASS, CRYPT, LOGGER, UPTOBOX_TOKEN
...
def direct_link_generator(link: str):
    ...
    if "youtube.com" in link or "youtu.be" in link:
        raise DirectDownloadLinkException(
            f"<b>⚠⁉ ERROR: </b>Mirroring/Leeching YouTube Videos and links was restricted"
        )
...
def account_login(client, url, email, password):
    ...
    if APPDRIVE_EMAIL is None:
        raise DirectDownloadLinkException(
            "ERROR: Appdrive  Email Password not provided"
        )
