...
"""
        ...
        sendMarkup(
            "Not Authorized user, Join our Mirror Group to Access BoTs!",
            context.bot,
            update.message,
            reply_markup,
        )
...
def restart(update, context):
    ...
    if (not HEROKU_API_KEY) or (not HEROKU_APP_NAME):
        LOGGER.info(
            "If you want Heroku features, fill HEROKU_APP_NAME HEROKU_API_KEY vars."
        )
        dynoRestart = False
        dynoKill = False
    if dynoRestart:
...
def main():
    ...
    hstats_handler = CommandHandler(
        BotCommands.HStatsCommand,
        hstats,
        filters=CustomFilters.owner_filter | CustomFilters.sudo_user,
        run_async=True,
    )
