...
from bot import  bot, Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                download_dict, download_dict_lock, TG_SPLIT_SIZE, LOGGER, MEGA_KEY, DB_URI, INCOMPLETE_TASK_NOTIFIER, \
                    LEECH_LOG, BOT_PM, MIRROR_LOGS, FSUB_CHANNEL_ID, FSUB, CHANNEL_USERNAME
...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        if self.isLeech:
            ...
            msg += f'\n\n<b>Elapsed : </b>{get_readable_time(time() - self.message.date.timestamp())} | <b>#cc : </b>{self.tag}\n\n'
            if not files:
                sendMessage(msg, self.bot, self.message)
            else:
                fmsg = '\n<b>Leeched Files Are:</b>\n'
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 4000:
                        ...
                        fmsg = ''
                if fmsg != '':
                    msg = sendMarkup(msg + fmsg, self.bot, self.message, InlineKeyboardMarkup(buttons.build_menu(2)))
        else:
            msg += f'\n\n<b>Type: </b>{typ}'
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0):
    ...
    if FSUB:
        try:
            user = bot.get_chat_member(
                f"{FSUB_CHANNEL_ID}", message.from_user.id
            )
    ...
    if BOT_PM and message.chat.type != 'private':
        try:
            ...
            startwarn = f"<b>Hey </b>{uname},\n\n<b>I found that you haven't started me in PM (Private Chat) yet.</b>\n\n" \
                        f"<b>From now on I will give <u>GDrive links also in PM</u></b>"
